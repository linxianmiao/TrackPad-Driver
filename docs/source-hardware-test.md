...
第一组 `TouchPackets=Reports=2887`，第二组增至 4261；
`InvalidPackets=0`，`Reconnects` 始终为 1，未在本轮功能测试中新增重连。

本轮分两组向用户说明操作，用户对每组均回复“正常”。结果如下；功能效果来自用户确认，
驱动计数作为数据链证据，不能单凭计数推断系统手势效果。

| 测试动作 | 预期效果 | 本机结果 |
| --- | --- | --- |
| 单指移动、按压 | 指针移动、点击 | 用户确认通过 |
| 双指上下滑、捏合/张开 | 网页滚动、缩放 | 用户确认通过 |
| 三指上滑、下滑 | 任务视图、显示桌面 | 用户确认通过 |
| 三指左右滑 | 切换窗口 | 用户确认通过 |
| 四指左右滑 | 切换虚拟桌面 | 用户确认通过 |

功能验收范围是本机 Windows 11 x64 和 Bluetooth `004c:0324`，版本 0.2.0.2。
睡眠/唤醒、禁用/启用、断连/重连、卸载恢复与 Verifier 尚未验收；不扩展为正式发布结论。
